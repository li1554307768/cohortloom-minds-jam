...
SENTINEL_PATTERN = re.compile(r"\bcl-continuity-[0-9a-f]{24}\b")
...
def random_alias(label: str) -> str:
    return f"cl-{label}-{secrets.token_hex(6)}"
...
def read_checkpoint_entries(
    ...
    for index, raw_item in enumerate(payload):
        ...
        if "stage" not in item and index == 0 and item.get("operation") == "store_hypothesis":
            item["stage"] = "store"
        ...
        expected_operation = {
            ...
            "recall_a": "recall_and_plan",
            "recall_b": "recall_and_review",
        }[str(stage)]
...
async def recover_store_from_official_history(
    ...
    if recovered.packet.operation != "store_hypothesis":
        raise MindsError("恢复出站请求不是 store_hypothesis")
    if recovered.parsed_response.get("stored") is not True:
        raise MindsError("store 官方回复未确认 stored=true")
    return recovered


def load_synthetic_weekly_summaries() -> tuple[str, str, list[dict[str, Any]]]:
    path = ROOT / "data" / "synthetic" / "audience_week.json"
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise MindsError("无法读取本地合成受众周摘要") from exc
    if not isinstance(payload, dict):
        raise MindsError("本地合成数据必须是 JSON 对象")
    summaries = payload.get("weekly_summaries")
    week_label = payload.get("week_label")
    experiment_goal = payload.get("experiment_goal")
    if (
        payload.get("dataset_label") != "SYNTHETIC_DEMO_ONLY"
        or not isinstance(summaries, list)
        or not isinstance(week_label, str)
        or not isinstance(experiment_goal, str)
    ):
        raise MindsError("本地周摘要不是明确标注的合成数据")
    result: list[dict[str, Any]] = []
    for item in summaries:
        if not isinstance(item, dict):
            raise MindsError("合成周摘要格式无效")
        result.append(
            {
    ...
    if len(result) != 3:
        raise MindsError("恢复证据需要恰好三个合成平台周摘要")
    return week_label, experiment_goal, result


def build_fresh_proof_packets() -> tuple[str, str, MindsPacket, list[MindsPacket]]:
    memory_key = f"cohortloom:hypothesis:quiet_creators:{secrets.token_hex(6)}"
    sentinel = f"cl-continuity-{secrets.token_hex(12)}"
    hypothesis = (
        "Practical teardown content activates quiet viewers more reliably than broad "
        f"motivational content. Continuity marker: {sentinel}."
    )
    week_label, experiment_goal, summaries = load_synthetic_weekly_summaries()
    store = build_store_packet(
        memory_key,
        segment_key="quiet_practical_creators",
        audience_hypothesis=hypothesis,
        evidence_basis=(
            "Synthetic saves and qualified replies were stronger on practical teardown content."
        ),
        risk_note="Synthetic small sample; run a bounded test before generalizing.",
    )
    recalls = [
        build_recall_packet(
            memory_key,
        ...
        build_due_check_packet(
            memory_key,
            experiment_ref="synthetic-growth-experiment-week-32",
            due_label="Synthetic Week 33 local due check",
            observed_result=(
                "The experiment is unfinished; two creator-owned actions were reviewed and no "
                "success threshold has been reached."
            ),
            success_condition="At least 12 qualified replies with save rate at or above 3%.",
            stop_condition=(
                "Stop if qualified replies stay below 4 or negative feedback exceeds 5%."
            ),
        ),
    ]
    return hypothesis, sentinel, store, recalls
...
def build_pending_recall_packets(
    ...
    allowed_data_keys = {
        frozenset({"segment_key", "approved_hypothesis", "evidence_basis", "risk_note"})
    }
    if frozenset(data) not in allowed_data_keys:
        raise MindsError("store 自包含 data 字段不在受控兼容集合")
    segment_key = data.get("segment_key")
    hypothesis = data.get("approved_hypothesis")
    if segment_key != "quiet_practical_creators":
        raise MindsError("store segment_key 不是预期的合成受众段")
    if not isinstance(hypothesis, str) or not hypothesis.strip():
        raise MindsError("store 缺少已批准受众假设")
    if not isinstance(data.get("evidence_basis"), str) or not isinstance(
        data.get("risk_note"), str
    ):
        raise MindsError("store 证据或风险边界无效")
    sentinels = SENTINEL_PATTERN.findall(hypothesis)
    if len(sentinels) != 1:
        raise MindsError("store 已批准假设缺少唯一不可猜测连续性标记")
    week_label, experiment_goal, summaries = load_synthetic_weekly_summaries()
    packets = [
        build_recall_packet(
    ...
    if [packet.operation for packet in packets] != [
        "recall_and_plan",
        "recall_and_review",
    ]:
        raise MindsError("恢复模式必须构建 plan recall 和 due-review recall")
    return hypothesis, sentinels[0], packets
...
def assert_continuity_recall(
    parsed: dict[str, Any], approved_hypothesis: str, sentinel: str
) -> None:
    ...
    full_hypothesis_match = normalize_proof_text(approved_hypothesis) in normalized
    if not sentinel_match and not full_hypothesis_match:
...
async def send_pending_recalls(
    ...
    approved_hypothesis: str,
    sentinel: str,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    if len(packets) != 2 or [packet.operation for packet in packets] != [
        "recall_and_plan",
        "recall_and_review",
    ]:
        raise MindsError("恢复模式必须且只能发送 plan + due-review 两个 recall")
    ...
    for packet, alias, stage in zip(packets, aliases, stages, strict=True):
        ...
        try:
            assert_continuity_recall(parsed, approved_hypothesis, sentinel)
        except MindsError:
...
def recovered_store_evidence(recovered: RecoveredStore) -> dict[str, Any]:
    ...
    evidence["operation"] = "store_hypothesis"
    return evidence
...
async def execute_mode(*, confirm_live: bool) -> int:
    ...
    api_key = os.getenv("COHORTLOOM_MINDS_API_KEY") or os.getenv("MINDS_BUILDER_API_KEY", "")
    mind_id = os.getenv("COHORTLOOM_MIND_ID") or os.getenv("MINDS_MIND_ID", "")
    base_url = os.getenv(
        "COHORTLOOM_MINDS_BASE_URL", "https://api.build.hellominds.ai"
    )
    ...
    if confirm_live:
        approved_hypothesis, sentinel, store_packet, packets = build_fresh_proof_packets()
        ...
        approved_hypothesis, sentinel, packets = build_pending_recall_packets(recovered)
    ...
    recall_calls, responses = await send_pending_recalls(
        transport, packets, approved_hypothesis, sentinel
    )
    ...
    evidence = {
        ...
        "new_sessions_receive_hypothesis_memory_key_and_bounded_weekly_summaries": True,
        "auto_post": False,
        "auto_recharge": False,
        ...
        "store_messages_sent_this_run": sum(
            1
            for call in calls
            if call["operation"] == "store_hypothesis" and call.get("sent_this_run") is True
        ),
        "recall_messages_sent_this_run": sum(
            1
            for call in calls
            if str(call["operation"]).startswith("recall_and_")
